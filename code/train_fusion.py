...
from libs.modules.fusion import Query, Merge, FocalLoss, SparseTransform
...
def main(args):
    ...
    file_list = '/media/feihu/Storage/kitti_point_cloud/semantic_kitti/train4.list'
...
#    net = MODEL.get_model(num_classes, normal_channel=args.normal)
    ...
    try:
        checkpoint = torch.load(str(experiment_dir) + '/checkpoints/model_iter4.2.pth')
...
#        for iteration, data in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
        for iteration, data in enumerate(trainDataLoader):
            ...
            if (iteration)%lr_step==0:
                lr_count = epoch*6 + (iteration)/lr_step
                lr = args.learning_rate * math.exp((1 - lr_count) * args.lr_decay)
#                lr = 0.001
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr
                log_string('Learning rate:%f' % lr)
...
#                evaluate()
                ...
                net.train()
                momentum = MOMENTUM_ORIGINAL * (MOMENTUM_DECCAY ** ((epoch + iteration/test_step) // MOMENTUM_DECCAY_STEP))
#                momentum = MOMENTUM_ORIGINAL * (MOMENTUM_DECCAY ** ((epoch *len(trainDataLoader)/test_step + iteration/test_step) // MOMENTUM_DECCAY_STEP))
#                momentum = 0.05
                if momentum < 0.01:
                    momentum = 0.01
...
#                    feas = points[start:end,:,3:].to(devices[i])
                    ...
                    sparse, org, label, maski, offsets = input_layer(pc, targeti, maski, scale.to(devices[i]), spatialSize.to(devices[i]))
#                        print(locs.size(), feas.size(), label.size(), maski.size(), offsets.size())
                    #org_coords = locs[1]
                    label = Variable(label, requires_grad=False)


                    inputi = ME.SparseTensor(sparse[1].cpu(), sparse[0].cpu()) 
                    inputs.append([inputi.to(devices[i]), org[0], org[1], maski])
...
#                temp = output_layer(locs, predictions[i]['output1'].F, coords[i])
 ...
 #               print(prediction2[i].size(), prediction1[i].size(), targets[i].size())
                outputi = results[i] #prediction2[i].contiguous().view(-1, num_classes)
...
#            print(prediction2, labels)
            losses = parallel.parallel_apply(criterions, tuple(zip(results, labels)), devices=devices)
...
#            log_string("===> Epoch[{}]({}/{}) Valid points:{}/{} Loss: {:.4f} Accuracy: {:.4f}".format(epoch, iteration, len(trainDataLoader), count, total_points, loss.item(), match/count))
            log_string("Epoch[{}]({}/{}) Valid:{}/{} Loss:{:.4f} Acc:{:.4f} Mean:{:.4f}".format(epoch, iteration, len(trainDataLoader), count, total_points, loss.item(), match/count, np.mean(mean_correct)))
#            sys.stdout.flush()
#        train_instance_acc = np.mean(mean_correct)
#        log_string('Train accuracy is: %.5f' % train_instance_acc)

#        continue
            if ((iteration+1)%test_step==0) or ((iteration+1)==len(trainDataLoader)):
                savepath = str(checkpoints_dir) + '/model_iter'+ str(epoch+(iteration+1)/test_step/10) + '.pth'
...
#                        feas = points[start:end,:,3:].to(devices[i])
                                ...
                                sparse, org, label, maski, offsets = input_layer(pc, targeti, maski, scale.to(devices[i]), spatialSize.to(devices[i]), False)
#                        print(locs.size(), feas.size(), label.size(), maski.size(), offsets.size())
                     #   org_coords = locs[1]
                                label = Variable(label, requires_grad=False)
...
#                    logger.info('Save model...')
                    ...
                    savepath = str(checkpoints_dir) + '/best_model_attention.pth'
                    log_string('Saving at %s'% savepath)
                    state = {
                        'epoch': epoch,
                        'test_acc': m_accuracy,
