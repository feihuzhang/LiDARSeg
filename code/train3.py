...
#adjust learing rate.
            if (iteration)%320==0:
                lr_count = 10 + (epoch-10)*4 + (iteration)/320
...
#            print(total_points)
            ...
            for i in range(num_devices):
                ...
                batch['x'][0]=batch['x'][0].type(torch.LongTensor)
                batch['y']=batch['y'].type(torch.LongTensor)
                label = Variable(batch['y'], requires_grad=False)
                maski = batch['mask'].type(torch.LongTensor)
...
#                print(locs.size(), feas.size(), batch['x'][0].size())
 ...
 #               print(inputi.size(), batch['x'][1].size())
                ...
                with torch.cuda.device(devices[i]):
                    ...
                    inputi = ME.SparseTensor(feas.cpu(), locs.cpu()) #input_coding(batch['x'])
#                    inputi = {x}
#                    print(inputi)
...
#            print("end ...") 
 ...
 #               temp = predictions[i]['output1'].F#.view(-1, num_classes)
                temp = predictions[i]['output1']
#                temp = output_layer(locs, predictions[i]['output1'].F, coords[i])
                temp = temp[targets[i] > 0, :]
                prediction1.append(temp)
#                locs = predictions[i]['output2'].C.to(devices[i])
                temp = predictions[i]['output2']#view(-1, num_classes)
#                temp = output_layer(locs, predictions[i]['output2'].F, coords[i])
                temp = temp[targets[i] > 0, :]
